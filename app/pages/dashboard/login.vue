...
import { z } from 'zod'
import { internalLogin, setAuthToken } from '~/services/auth'
...
const handleSubmit = async () => {
  ...
  try {
    const response = await internalLogin(payload)
    const accessToken = response?.data?.access_token

    if (!accessToken) {
      throw new Error('Access token not found in internal login response')
    }

    setAuthToken(accessToken, response.data.token_type || 'Bearer', 'internal')
